...
def main() -> None:
    ...
    for key in sorted(latest, key=lambda k: ({"depth": 0, "struct": 1, "choice": 2}[k[0]], k[2], k[1])):
        ...
        rows.append({
            ...
            "process_wall_seconds": data.get("verify_wall_seconds"),
    ...
    lines = [
        ...
        "Depth is a chain of single-field records; structure width is the number "
        "of `u8` fields in one record; choice width is the number of tag-disjoint "
        "`u8` alternatives in one choice. Measurements use a warm shared target "
        "cache, 10 workers, a 300 s limit, and Rust recursion limit 512.", "",
    ...
    for r in rows:
        ...
        lines.append(
            ...
            f'{number("verification_wall_seconds")} | {number("process_wall_seconds")} | '
            f'{number("smt_cpu_seconds")} | {r["result"]} |'
        )
    report = "\n".join(lines) + "\n"
