#!/usr/bin/env python3
"""Produce reproducible source/proof-burden statistics for the VPS backend."""
...
class Stats:
    ...
    def row(self) -> dict[str, object]:
        result = asdict(self)
        # Shared declarations (imports, datatypes, trait declarations, etc.)
        # support all three kinds of code. Apportion them as evenly as integer
        # SLOC permit so the reported Spec + Proof + Impl columns cover the
        # complete source. Preserve the direct lexical counts for auditing.
        direct_spec = self.spec_sloc
        direct_proof = self.proof_sloc
        direct_exec = self.exec_sloc
        share, remainder = divmod(self.shared_sloc, 3)
        result["spec_sloc"] = direct_spec + share + (remainder >= 1)
        result["proof_sloc"] = direct_proof + share + (remainder >= 2)
        result["exec_sloc"] = direct_exec + share
        result["direct_spec_sloc"] = direct_spec
        result["direct_proof_sloc"] = direct_proof
        result["direct_exec_sloc"] = direct_exec
        result["proof_to_code"] = (
            (result["spec_sloc"] + result["proof_sloc"]) / result["exec_sloc"]
            if result["exec_sloc"]
            else None
        )
        assert (
...
def tracked_rs_files(repo: Path) -> list[Path]:
    output = subprocess.check_output(
        ["git", "ls-files", "vps-lib/src"], cwd=repo, text=True
    )
...
def main() -> None:
    ...
    for name in ("core", "combinators", "primitives"):
        if name in grouped:
            framework.add(grouped[name])
    case_studies = Stats("CASE_STUDIES")
    for name in ("asn1", "cbor"):
        if name in grouped:
            case_studies.add(grouped[name])

    # Keep detailed rows for reproducibility, but expose explicit subtotals so
    # papers do not conflate the reusable framework with its case studies.
    stats_rows = (
        [grouped[k].row() for k in order if k in grouped]
        + [framework.row(), case_studies.row(), total.row()]
    )
    ...
    with (args.output_dir / "backend_loc.csv").open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=stats_rows[0].keys(), lineterminator="\n")
        writer.writeheader()
