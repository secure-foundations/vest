...
VPS develops a new theory of format ambiguity that is more flexible and yet *subsumes* Vest1.0. The key is to specify the serialization functions in destination-passing style (DPS):
...
- Encodes the "True" permutation semantics
  ...
  - Although VPS would reject ambiguous grammars like these for any combinators implementing (`SPRoundtrip`)
...
- Because this encoding is non-backtracking, it does not actually encode the "True" permutation semantics.
  - Maybe we can prove that given unambiguous fields (VPS already enforces this for any combinator implementing `SPRoundtrip`), two versions of permutations are semantically equivalent
