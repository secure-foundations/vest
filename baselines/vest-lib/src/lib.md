...
`vest_lib` is a *formally verified* combinator library for parsing and serializing binary data. It is the backend of the Vest framework, which can automatically produce verified parsers and serializers from a high-level format description. This copy is retained as the historical evaluation baseline.

This document primarily serves as a reference for users who want to use the combinator library directly to achieve greater flexibility in the parsing and serialization process. The bundled `baselines/vest-dsl` directory contains the corresponding Vest DSL compiler and case studies.
