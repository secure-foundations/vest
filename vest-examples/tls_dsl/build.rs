...
fn main() {
    // Tell Cargo to rerun this build script if src/tls.vest changes
    println!("cargo:rerun-if-changed=src/tls.vest");

    // Get the output directory where generated files should be placed
    let out_dir = env::var("OUT_DIR").expect("OUT_DIR environment variable not set");
    let out_path = PathBuf::from(&out_dir).join("tls.rs");

    // Run the vest executable on src/tls.vest
