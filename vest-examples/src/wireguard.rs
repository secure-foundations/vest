...
spec fn wg_msg1() -> (
    Tag<BytesN<4>, Seq<u8>, Seq<u8>>,
    (Bytes, (Bytes, (Bytes, (Bytes, (Bytes, Tag<BytesN<16>, Seq<u8>, Seq<u8>>))))),
) {
    let grouplen = 32;
    let cipherlen_group = 48;
    let cipherlen_12 = 28;
    let maclen = 16;
    let tag = Tag::spec_new(BytesN::<4>, SPEC_BYTES_1000_CONST, SPEC_BYTES_1000_CONST);
    let sender = Bytes(4);
    let eph = Bytes(grouplen);
    let statik = Bytes(cipherlen_group);
    let timestamp = Bytes(cipherlen_12);
    let mac1 = Bytes(maclen);
    let mac2 = Tag::spec_new(BytesN::<16>, SPEC_BYTES_16_0S_CONST, SPEC_BYTES_16_0S_CONST);
    (tag, (sender, (eph, (statik, (timestamp, (mac1, mac2))))))
}

fn mk_wg_msg1<'x>() -> (res: (
    Tag<BytesN<4>, [u8; 4], &'x [u8]>,
    (Bytes, (Bytes, (Bytes, (Bytes, (Bytes, Tag<BytesN<16>, [u8; 16], &'x [u8]>))))),
))
    ensures
        res@ == wg_msg1(),
{
    ...
    let cipherlen_12 = 28;
    let maclen = 16;
    let tag = Tag::new(BytesN::<4>, BYTES_1000_CONST, BYTES_1000_CONST.as_slice());
    let sender = Bytes(4);
    let eph = Bytes(grouplen);
    let statik = Bytes(cipherlen_group);
    let timestamp = Bytes(cipherlen_12);
    let mac1 = Bytes(maclen);
    let mac2 = Tag::new(BytesN::<16>, BYTES_16_0S_CONST, BYTES_16_0S_CONST.as_slice());
    (tag, (sender, (eph, (statik, (timestamp, (mac1, mac2))))))
}
...
//     let cipherlen_12 = 28;
//     let maclen = 16;
//     let tag = Tag::new(BytesN::<4>, BYTES_1000_CONST, BYTES_1000_CONST);
//     let sender = Bytes(4);
//     let eph = Bytes(grouplen);
//     let statik = BuilderCombinator(ExampleBuilder);
//     let timestamp = Bytes(cipherlen_12);
//     let mac1 = Bytes(maclen);
//     let mac2 = Tag::new(BytesN::<16>, BYTES_16_0S_CONST, BYTES_1000_CONST);
//     (tag, (sender, (eph, (statik, (timestamp, (mac1, mac2))))))
// }
