...
fn main() {
    // Tell Cargo to rerun this build script if src/mavlink.vest changes
    println!("cargo:rerun-if-changed=src/mavlink.vest");

    // Get the output directory where generated files should be placed
    let out_dir = env::var("OUT_DIR").expect("OUT_DIR environment variable not set");
    let out_path = PathBuf::from(&out_dir).join("mavlink.rs");

    // Run the vest executable on src/mavlink.vest
    let status = Command::new("vest")
        .arg("src/mavlink.vest")
        .arg("--output")
        .arg(&out_path)
        .status()
