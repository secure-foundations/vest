use crate::vestir::{
    ...
    ConstraintEnumCombinator, ConstraintIntCombinator, ConstCombinator, Definition, Endianess,
    EnumCombinator, GlobalCtx, LengthExpr, OptionCombinator, ParamDefn, StructCombinator,
...
impl<'a> Analysis<'a> {
    pub(crate) fn new(defs: &'a [Definition], ctx: &'a GlobalCtx) -> Self {
        ...
        let mut this = Self { defs, ctx, endianness, infos: HashMap::new() };
        for def in defs {
            if let Some(name) = definition_name(def) {
                ...
                this.infos.insert(
                    name.to_string(),
                    FormatInfo { names, needs_lifetime, non_tail, non_malleable },
                );
            }
        }
        this
    ...
    pub(crate) fn info(&self, name: &str) -> &FormatInfo {
        self.infos.get(name).unwrap_or_else(|| panic!("missing format info for `{name}`"))
    }
    ...
    pub(crate) fn render_inner_type(
        ...
        match self.ctx.resolve_alias(inner) {
            ...
            CombinatorInner::Struct(struct_comb) => {
                if top_level {
                    self.nominal_type(&self.definition_name_for_inner(inner), mode)
                } else {
    ...
    pub(crate) fn choice_variant_names(&self, choice_comb: &ChoiceCombinator) -> Vec<String> {
        match &choice_comb.choices {
            Choices::Enums(branches) => branches
                .iter()
                .map(|(name, _)| if name == "_" { "Default".to_string() } else { name.clone() })
                .collect(),
    ...
    pub(crate) fn render_const_value_type(
        ...
        match self.ctx.resolve_const(combinator) {
            ...
            ConstCombinator::ConstEnum(enum_comb) => self.nominal_type(&enum_comb.combinator.func, mode),
    ...
    pub(crate) fn definition_name_for_inner(&self, inner: &CombinatorInner) -> String {
        match inner {
            CombinatorInner::Invocation(invocation) => invocation.func.clone(),
            _ => self
                .defs
                .iter()
                .find_map(|def| match def {
                    Definition::Combinator { name, combinator, .. }
                        if std::ptr::eq(&combinator.inner, inner) =>
                    {
                        Some(name.clone())
                    },
                    _ => None,
    ...
    pub(crate) fn param_defns_for(&self, name: &str) -> &[ParamDefn] {
        self.defs
            .iter()
            .find_map(|def| match def {
                Definition::Combinator { name: def_name, param_defns, .. } if def_name == name => {
                    Some(param_defns.as_slice())
                },
                _ => None,
            })
            .unwrap_or(&[])
    ...
    fn definition_needs_lifetime(&self, def: &Definition) -> bool {
        match def {
            ...
            Definition::ConstCombinator { const_combinator, .. } => self.const_needs_lifetime(const_combinator),
    ...
    fn inner_needs_lifetime(&self, inner: &CombinatorInner) -> bool {
        match self.ctx.resolve_alias(inner) {
            ...
            CombinatorInner::Struct(StructCombinator(fields)) => fields.iter().any(|field| match field {
                StructField::Const { combinator, .. } => self.const_needs_lifetime(combinator),
                StructField::Dependent { combinator, .. }
                | StructField::Ordinary { combinator, .. } => self.combinator_needs_lifetime(combinator),
            }),
            ...
            CombinatorInner::Choice(choice) => match &choice.choices {
                Choices::Enums(branches) => branches.iter().any(|(_, combinator)| self.combinator_needs_lifetime(combinator)),
                Choices::Ints(branches) => branches.iter().any(|(_, combinator)| self.combinator_needs_lifetime(combinator)),
                Choices::Arrays(branches) => branches.iter().any(|(_, combinator)| self.combinator_needs_lifetime(combinator)),
            },
            CombinatorInner::Vec(VecCombinator::Vec(combinator)) => self.combinator_needs_lifetime(combinator),
            CombinatorInner::Array(ArrayCombinator { combinator, .. }) => self.combinator_needs_lifetime(combinator),
            CombinatorInner::Bytes(_) | CombinatorInner::Tail(_) => true,
            CombinatorInner::Option(OptionCombinator(combinator)) => self.combinator_needs_lifetime(combinator),
    ...
    fn const_non_tail(&self, combinator: &ConstCombinator) -> bool {
        match self.ctx.resolve_const(combinator) {
            ConstCombinator::ConstBytes(_) | ConstCombinator::ConstInt(_) | ConstCombinator::ConstEnum(_) => true,
    ...
    fn definition_non_malleable(&self, def: &Definition) -> bool {
        match def {
            ...
            Definition::ConstCombinator { const_combinator, .. } => self.const_non_malleable(const_combinator),
            Definition::Endianess(_) => true,
        }
    }
    ...
    fn combinator_non_malleable(&self, combinator: &Combinator) -> bool {
        ...
        match self.ctx.resolve_alias(&combinator.inner) {
            ...
            CombinatorInner::Choice(choice) => match &choice.choices {
                Choices::Enums(branches) => branches.iter().all(|(_, combinator)| self.combinator_non_malleable(combinator)),
                Choices::Ints(branches) => branches.iter().all(|(_, combinator)| self.combinator_non_malleable(combinator)),
                Choices::Arrays(branches) => branches.iter().all(|(_, combinator)| self.combinator_non_malleable(combinator)),
            },
            CombinatorInner::Tail(_) => true,
            CombinatorInner::Vec(VecCombinator::Vec(combinator)) => self.combinator_non_malleable(combinator),
            CombinatorInner::Array(ArrayCombinator { combinator, .. }) => self.combinator_non_malleable(combinator),
            CombinatorInner::Option(OptionCombinator(combinator)) => self.combinator_non_malleable(combinator),
