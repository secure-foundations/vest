...
impl<'i> DeepView for NestedInnerStructV<'i> {
    ...
    open spec fn deep_view(&self) -> Self::V {
        NestedInnerStructVSpec { x: self.x.deep_view(), y: self.y.deep_view() }
    }
}
...
impl<'i> DeepView for NestedInnerStruct<'i> {
    ...
    open spec fn deep_view(&self) -> Self::V {
        NestedInnerStructSpec { len: self.len.deep_view(), v: self.v.deep_view() }
    }
}
...
pub open spec fn capture_param_and_local_x_a_payload_fmt(
    choice2: COrDSpec,
    len: u8,
    ...
    Named(
        ...
        Mapped {
            ...
            mapper: (
                |parsed: CaptureParamAndLocalXAPayloadInner| -> CaptureParamAndLocalXAPayloadSpec
                    {
                        match parsed {
...
pub open spec fn capture_param_and_local_x_a_fmt(
    choice2: COrDSpec,
) -> CaptureParamAndLocalXAFmtSpec {
    Named(
        "capture_param_and_local_x_a",
        Mapped {
            inner: Bind(U8, |len: u8| CaptureParamAndLocalXAPayloadFmt { choice2, len }),
            mapper: (
...
pub open spec fn capture_param_and_local_x_b_fmt() -> CaptureParamAndLocalXBFmtSpec {
    Named(
        ...
        Mapped {
            inner: Bind(U8, |tag: u8| CaptureParamAndLocalXBYFmt { tag }),
            mapper: (
...
pub open spec fn capture_param_and_local_x_fmt(
    choice1: AOrBSpec,
    choice2: COrDSpec,
    ...
    Named(
        ...
        Mapped {
            inner: match choice1 {
                AOrBSpec::A => Sum::Inl(CaptureParamAndLocalXAFmt { choice2 }),
...
pub type NestedInnerStructVFmtSpec = Named<
    Mapped<Pair<U8, Tail>, FnSpecMapper<NestedInnerStructVInner, NestedInnerStructVSpec>>,
>;
...
pub open spec fn nested_inner_struct_v_fmt() -> NestedInnerStructVFmtSpec {
    Named(
        "nested_inner_struct_v",
        Mapped {
            inner: Pair(U8, Tail),
            mapper: (
                |parsed: NestedInnerStructVInner| -> NestedInnerStructVSpec
                    {
                        let (x, y) = parsed;
                        NestedInnerStructVSpec { x, y }
                    },
                |value: NestedInnerStructVSpec| -> NestedInnerStructVInner
                    {
                        let NestedInnerStructVSpec { x, y } = value;
                        (x, y)
                    },
            ),
        },
...
pub open spec fn capture_outer_and_local_payload_body_fmt(
    frame_len: u8,
    tag: u8,
) -> CaptureOuterAndLocalPayloadBodyFmtSpec {
    Named(
...
pub open spec fn capture_outer_and_local_payload_fmt(
    frame_len: u8,
) -> CaptureOuterAndLocalPayloadFmtSpec {
    Named(
        "capture_outer_and_local_payload",
        Mapped {
            inner: Bind(U8, |tag: u8| CaptureOuterAndLocalPayloadBodyFmt { frame_len, tag }),
            mapper: (
    ...
    Named(
        ...
        Mapped {
            inner: Bind(
                Refined(U8, |x: u8| x >= 1),
                |frame_len: u8|
                    ExactLen((frame_len as usize), CaptureOuterAndLocalPayloadFmt { frame_len }),
            ),
            mapper: (
        ...
        Mapped {
            ...
            mapper: (
                ...
                |value: CaptureLocalInAnonStructWrapperValueChoice0Spec|
                 -> CaptureLocalInAnonStructWrapperValueChoice0Inner
                    {
                        let CaptureLocalInAnonStructWrapperValueChoice0Spec { len, bytes } = value;
                        (len, bytes)
                    },
            ),
        },
...
pub open spec fn capture_local_in_anon_struct_wrapper_value_fmt(
    tag: u8,
) -> CaptureLocalInAnonStructWrapperValueFmtSpec {
    Named(
...
pub open spec fn nested_inner_choice_x_fmt(
    choice1: AOrBSpec,
    choice2: COrDSpec,
) -> NestedInnerChoiceXFmtSpec {
    Named(
        "nested_inner_choice_x",
        Mapped {
            inner: match choice1 {
                AOrBSpec::A => Sum::Inl(NestedInnerChoiceXAFmt { choice2 }),
...
pub open spec fn nested_inner_choice_fmt(
    choice1: AOrBSpec,
    choice2: COrDSpec,
) -> NestedInnerChoiceFmtSpec {
    Named(
        "nested_inner_choice",
        Mapped {
            inner: NestedInnerChoiceXFmt { choice1, choice2 },
            mapper: (
...
pub open spec fn capture_param_and_local_fmt(
    choice1: AOrBSpec,
    choice2: COrDSpec,
    ...
    Named(
        ...
        Mapped {
            inner: CaptureParamAndLocalXFmt { choice1, choice2 },
            mapper: (
...
pub type NestedInnerStructFmtSpec = Named<
    Mapped<
        Bind<U32Le, spec_fn(u32) -> ExactLen<NestedInnerStructVFmt, usize>>,
...
pub open spec fn nested_inner_struct_fmt() -> NestedInnerStructFmtSpec {
    Named(
        "nested_inner_struct",
        Mapped {
            inner: Bind(U32Le, |len: u32| ExactLen((len as usize), NestedInnerStructVFmt)),
            mapper: (
                |parsed: NestedInnerStructInner| -> NestedInnerStructSpec
                    {
                        let (len, v) = parsed;
                        NestedInnerStructSpec { len, v }
                    },
                |value: NestedInnerStructSpec| -> NestedInnerStructInner
                    {
                        let NestedInnerStructSpec { len, v } = value;
                        (len, v)
                    },
            ),
        },
...
pub open spec fn capture_local_in_anon_struct_wrapper_fmt() -> CaptureLocalInAnonStructWrapperFmtSpec {
    Named(
        ...
        Mapped {
            inner: Bind(U8, |tag: u8| CaptureLocalInAnonStructWrapperValueFmt { tag }),
            mapper: (
...
mod derived_specs {
    ...
    impl SpecParser for CaptureParamAndLocalXAPayloadFmt {
        ...
        open spec fn spec_parse(&self, ibuf: Seq<u8>) -> Option<(int, Self::PVal)> {
            capture_param_and_local_x_a_payload_fmt(
    ...
    impl SpecSerializerDps for CaptureParamAndLocalXAFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_param_and_local_x_a_fmt(self.choice2.deep_view()).spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for CaptureParamAndLocalXBYFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_param_and_local_x_b_y_fmt(self.tag.deep_view()).spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for CaptureParamAndLocalXBFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_param_and_local_x_b_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for NestedInnerChoiceXAFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            nested_inner_choice_x_a_fmt(self.choice2.deep_view()).spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for NestedInnerStructVFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            nested_inner_struct_v_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for CaptureOuterAndLocalPayloadBodyChoice1Fmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_outer_and_local_payload_body_choice1_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecParser for CaptureOuterAndLocalPayloadBodyFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_parse(&self, ibuf: Seq<u8>) -> Option<(int, Self::PVal)> {
            capture_outer_and_local_payload_body_fmt(
    ...
    impl SpecSerializerDps for CaptureOuterAndLocalPayloadFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_outer_and_local_payload_fmt(self.frame_len.deep_view()).spec_serialize_dps(
                v,
                obuf,
            )
        }
    ...
    impl SpecSerializerDps for CaptureOuterAndLocalFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_outer_and_local_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for CaptureLocalInAnonStructWrapperValueChoice0Fmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_local_in_anon_struct_wrapper_value_choice0_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for CaptureLocalInAnonStructWrapperValueFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_local_in_anon_struct_wrapper_value_fmt(self.tag.deep_view()).spec_serialize_dps(
                v,
                obuf,
            )
        }
    ...
    impl SpecByteLen for NestedInnerChoiceXFmt {
        ...
        # [verifier :: opaque]
        open spec fn byte_len(&self, v: Self::T) -> nat {
            nested_inner_choice_x_fmt(self.choice1.deep_view(), self.choice2.deep_view()).byte_len(
                v,
            )
        }
    }
    ...
    impl SpecParser for NestedInnerChoiceFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_parse(&self, ibuf: Seq<u8>) -> Option<(int, Self::PVal)> {
            nested_inner_choice_fmt(self.choice1.deep_view(), self.choice2.deep_view()).spec_parse(
                ibuf,
            )
        }
    }
    ...
    impl Consistency for NestedInnerChoiceFmt {
        ...
        # [verifier :: opaque]
        open spec fn consistent(&self, v: Self::Val) -> bool {
            nested_inner_choice_fmt(self.choice1.deep_view(), self.choice2.deep_view()).consistent(
                v,
            )
        }
    }
    ...
    impl SpecByteLen for NestedInnerChoiceFmt {
        ...
        # [verifier :: opaque]
        open spec fn byte_len(&self, v: Self::T) -> nat {
            nested_inner_choice_fmt(self.choice1.deep_view(), self.choice2.deep_view()).byte_len(v)
        }
    }
    ...
    impl SpecSerializerDps for NestedInnerStructFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            nested_inner_struct_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for COrDFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            c_or_d_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for CaptureLocalInAnonStructWrapperFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_local_in_anon_struct_wrapper_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for CaptureLocalInAnonStructFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            capture_local_in_anon_struct_fmt().spec_serialize_dps(v, obuf)
        }
    }
    ...
    impl SpecSerializerDps for AOrBFmt {
        ...
        # [verifier :: opaque]
        open spec fn spec_serialize_dps(&self, v: Self::SValue, obuf: Seq<u8>) -> Seq<u8> {
            a_or_b_fmt().spec_serialize_dps(v, obuf)
        }
    }
...
mod derived_proofs {
    ...
    impl Productive for CaptureParamAndLocalXAPayloadFmt {
        ...
        proof fn lemma_productive(&self, s: Seq<u8>) {
            reveal(<CaptureParamAndLocalXAPayloadFmt as SpecParser>::spec_parse);
            let fmt = capture_param_and_local_x_a_payload_fmt(
                self.choice2.deep_view(),
                self.len.deep_view(),
            );
    ...
    impl Productive for CaptureParamAndLocalXFmt {
        ...
        proof fn lemma_productive(&self, s: Seq<u8>) {
            reveal(<CaptureParamAndLocalXFmt as SpecParser>::spec_parse);
            let fmt = capture_param_and_local_x_fmt(
                self.choice1.deep_view(),
                self.choice2.deep_view(),
            );
            assert(fmt.productive_inv());
    ...
    impl Productive for CaptureOuterAndLocalPayloadBodyFmt {
        ...
        proof fn lemma_productive(&self, s: Seq<u8>) {
            reveal(<CaptureOuterAndLocalPayloadBodyFmt as SpecParser>::spec_parse);
            let fmt = capture_outer_and_local_payload_body_fmt(
                self.frame_len.deep_view(),
                self.tag.deep_view(),
            );
            assert(fmt.productive_inv());
    ...
    impl SafeParser for CaptureLocalInAnonStructWrapperValueFmt {
        proof fn lemma_parse_safe(&self, ibuf: Seq<u8>) {
            ...
            capture_local_in_anon_struct_wrapper_value_fmt(self.tag.deep_view()).lemma_parse_safe(
                ibuf,
            );
        }
    }
    ...
    impl Productive for NestedInnerChoiceXFmt {
        ...
        proof fn lemma_productive(&self, s: Seq<u8>) {
            reveal(<NestedInnerChoiceXFmt as SpecParser>::spec_parse);
            let fmt = nested_inner_choice_x_fmt(self.choice1.deep_view(), self.choice2.deep_view());
            assert(fmt.productive_inv());
            fmt.lemma_productive(s);
        }
    }
    ...
    impl Productive for NestedInnerChoiceFmt {
        ...
        proof fn lemma_productive(&self, s: Seq<u8>) {
            reveal(<NestedInnerChoiceFmt as SpecParser>::spec_parse);
            let fmt = nested_inner_choice_fmt(self.choice1.deep_view(), self.choice2.deep_view());
            assert(fmt.productive_inv());
            fmt.lemma_productive(s);
        }
    }
    ...
    impl Productive for CaptureParamAndLocalFmt {
        ...
        proof fn lemma_productive(&self, s: Seq<u8>) {
            reveal(<CaptureParamAndLocalFmt as SpecParser>::spec_parse);
            let fmt = capture_param_and_local_fmt(
                self.choice1.deep_view(),
                self.choice2.deep_view(),
            );
            assert(fmt.productive_inv());
...
impl<'i> Parser<&'i [u8]> for CaptureParamAndLocalXFmt {
    ...
    fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
        ...
        let (n, v) = match self.choice1 {
            AOrB::A => {
                let (n, v) = (CaptureParamAndLocalXAFmt { choice2: self.choice2 }).parse(&rest)?;
...
impl<'i> Parser<&'i [u8]> for NestedInnerStructVFmt {
    ...
    fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
        ...
        let rest = rest.skip(n2);
        let total_n = n1 + n2;
        let final_v = NestedInnerStructV { x, y };
        assert(self.spec_parse(ibuf@) == Some((total_n as int, final_v.deep_view())));
        Ok((total_n, final_v))
    }
}
...
impl<'i> Parser<&'i [u8]> for CaptureOuterAndLocalPayloadBodyFmt {
    ...
    fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
        ...
        let (n, v) = match self.tag {
            0 => {
                let (n, v) = (Varied((((self.frame_len as usize) - 1 as usize) as usize))).parse(
                    &rest,
                )?;
...
impl<'i> Parser<&'i [u8]> for NestedInnerStructFmt {
    ...
    fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
        ...
        let rest = rest.skip(n2);
        let total_n = n1 + n2;
        let final_v = NestedInnerStruct { len, v };
        assert(self.spec_parse(ibuf@) == Some((total_n as int, final_v.deep_view())));
        Ok((total_n, final_v))
    }
}
