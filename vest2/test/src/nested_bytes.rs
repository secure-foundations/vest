...
pub type FixedArrayOfBytes<'i> = [&'i [u8] ;
3] ;
...
impl AnythingFmt {
    # [doc = "specification constructor for `anything`."] pub open spec fn spec_inner() -> AnythingFmtSpec {
        Named ("anything",
        Mapped {
            inner: U8,
            mapper: (| parsed: AnythingInner | -> AnythingSpec {
                let x = parsed ;
                AnythingSpec {
                    x
                }
...
pub type NestedDynamicBytesFmtSpec = Named < Mapped < Bind < U16Le, spec_fn (u16) -> Bind < U16Le, spec_fn (u16) -> RepeatN < Varied < u16 >, u16 > > >, FnSpecMapper < NestedDynamicBytesInner, NestedDynamicBytesSpec >> > ;

impl NestedDynamicBytesFmt {
    # [doc = "specification constructor for `nested_dynamic_bytes`."] pub open spec fn spec_inner() -> NestedDynamicBytesFmtSpec {
        Named ("nested_dynamic_bytes",
        Mapped {
            inner: Bind (U16Le,
            | num: u16 | Bind (U16Le,
            | num_inner: u16 | RepeatN (num,
            Varied (num_inner)))),
            mapper: (| parsed: NestedDynamicBytesInner | -> NestedDynamicBytesSpec {
                let (num,
                (num_inner,
                xs)) = parsed ;
                NestedDynamicBytesSpec {
...
pub type NestedFixedBytesFmtSpec = Named < Mapped < Bind < U16Le, spec_fn (u16) -> RepeatN < Fixed < 10 >, u16 > >, FnSpecMapper < NestedFixedBytesInner, NestedFixedBytesSpec >> > ;

impl NestedFixedBytesFmt {
    # [doc = "specification constructor for `nested_fixed_bytes`."] pub open spec fn spec_inner() -> NestedFixedBytesFmtSpec {
        Named ("nested_fixed_bytes",
        Mapped {
            inner: Bind (U16Le,
            | num: u16 | RepeatN (num,
            Fixed::< 10 >)),
            mapper: (| parsed: NestedFixedBytesInner | -> NestedFixedBytesSpec {
                let (num,
                xs) = parsed ;
                NestedFixedBytesSpec {
                    num,
...
impl FixedArrayOfBytesFmt {
    # [doc = "specification constructor for `fixed_array_of_bytes`."] pub open spec fn spec_inner() -> FixedArrayOfBytesFmtSpec {
        Named ("fixed_array_of_bytes",
        Array::< 3,
        _ > (Fixed::< 2 >))
    }
}
...
impl VecOfBytesFmt {
    # [doc = "specification constructor for `vec_of_bytes`."] pub open spec fn spec_inner() -> VecOfBytesFmtSpec {
        Named ("vec_of_bytes",
        RepeatTillEnd (Fixed::< 2 >))
    }
}
...
impl OptionalBytesFmt {
    # [doc = "specification constructor for `optional_bytes`."] pub open spec fn spec_inner() -> OptionalBytesFmtSpec {
        Named ("optional_bytes",
        OptionalEnd (Fixed::< 2 >))
    }
}
...
pub type TailVecFmtSpec = Named < Mapped < AndThen < Tail, RepeatTillEnd < AnythingFmt > >, FnSpecMapper < TailVecInner, TailVecSpec >> > ;

impl TailVecFmt {
    # [doc = "specification constructor for `tail_vec`."] pub open spec fn spec_inner() -> TailVecFmtSpec {
        Named ("tail_vec",
        Mapped {
            inner: AndThen (Tail,
            RepeatTillEnd (AnythingFmt)),
            mapper: (| parsed: TailVecInner | -> TailVecSpec {
                let xs = parsed ;
                TailVecSpec {
                    xs
                }
...
mod exec_impls {
    ...
    impl<'i> Parser<&'i [u8]> for NestedDynamicBytesFmt {
        ...
        fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
            ...
            let final_v = NestedDynamicBytes {
                num,
                num_inner,
                xs,
            };
    ...
    impl<Output: OutputBuf, 'i> Serializer<Output, NestedDynamicBytes<'i>> for NestedDynamicBytesFmt {
