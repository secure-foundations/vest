...
verus! {
    ...
    impl ATypeFmt {
        # [doc = "specification constructor for `a_type`."] pub open spec fn spec_inner () -> ATypeFmtSpec {
            Named ("a_type" ,
            Mapped {
                inner : Refined (U8 ,
                | x : u8 | x == 0 || x == 1 || x == 2) ,
    ...
    impl Msg4VFmt {
        # [doc = "specification constructor for `msg4_v`."] pub open spec fn spec_inner (t : ATypeSpec) -> Msg4VFmtSpec {
            Named ("msg4_v" ,
            Mapped {
                inner : match t {
    ...
    impl Msg4Fmt {
        # [doc = "specification constructor for `msg4`."] pub open spec fn spec_inner () -> Msg4FmtSpec {
            ...
            Mapped {
                inner : Bind (ATypeFmt ,
                | t : ATypeSpec | Pair (Msg4VFmt :: spec (t) ,
                Tail)) ,
    ...
    mod derived_specs {
        ...
        impl SpecSerializerDps for ATypeFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ATypeFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecParser for Msg4VFmt {
            ...
            ibuf : Seq < u8 >) -> Option < (int ,
            Self :: PVal) > {
                Msg4VFmt :: spec_inner (self . t_spec ()) . spec_parse (ibuf)
            }
        }
        impl Consistency for Msg4VFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                Msg4VFmt :: spec_inner (self . t_spec ()) . consistent (v)
            }
        }
        impl SpecSerializerDps for Msg4VFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                Msg4VFmt :: spec_inner (self . t_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
        impl SpecSerializer for Msg4VFmt {
            ...
            # [verifier :: opaque] open spec fn spec_serialize (& self ,
            v : Self :: SVal) -> Seq < u8 > {
                Msg4VFmt :: spec_inner (self . t_spec ()) . spec_serialize (v)
            }
        }
        impl SpecByteLen for Msg4VFmt {
            ...
            # [verifier :: opaque] open spec fn byte_len (& self ,
            v : Self :: T) -> nat {
                Msg4VFmt :: spec_inner (self . t_spec ()) . byte_len (v)
            }
        }
    ...
    mod derived_proofs {
        ...
        impl SafeParser for Msg4VFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                Msg4VFmt :: spec_inner (self . t_spec ()) . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for Msg4VFmt {
            open spec fn productive_inv (& self) -> bool {
                Msg4VFmt :: spec_inner (self . t_spec ()) . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< Msg4VFmt as SpecParser > :: spec_parse) ;
                let fmt = Msg4VFmt :: spec_inner (self . t_spec ()) ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
