...
pub type Msg4VFmtSpec = Named<
    Mapped<Sum<Msg1Fmt, Sum<Msg2Fmt, Msg3Fmt>>, FnSpecMapper<Msg4VInner, Msg4VSpec>>,
>;
...
impl<'i> Parser<&'i [u8]> for Msg4VFmt {
    type PT = Msg4V<'i>;
...
impl<'i> Parser<&'i [u8]> for Msg4Fmt {
    ...
    fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
        ...
        let rest = rest.skip(n1);
        let (n2, v) = (Msg4VFmt { t: t }).parse(&rest)?;
        let rest = rest.skip(n2);
        let (n3, tail) = (Tail).parse(&rest)?;
        let rest = rest.skip(n3);
        let total_n = n1 + n2 + n3;
        let final_v = Msg4 { t, v, tail };
        assert(self.spec_parse(ibuf@) == Some((total_n as int, final_v.deep_view())));
        Ok((total_n, final_v))
    }
}
