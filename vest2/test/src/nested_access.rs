...
verus! {
    ...
    pub type NestedComplexFmtSpec = Named < Mapped < Pair < Const < U32Le , u32 > , Varied < usize > > , FnSpecMapper < NestedComplexInner , NestedComplexSpec >> > ;

    impl < 'i > NestedComplexFmt < 'i > {
        # [doc = "specification constructor for `nested_complex`."] pub open spec fn spec_inner (hdr_payload : PayloadWithHeaderSpec) -> NestedComplexFmtSpec {
            Named ("nested_complex" ,
            Mapped {
                inner : Pair (Const (U32Le ,
                0) ,
                Varied ((((hdr_payload . hdr . payload_length as usize) - 8) as usize))) ,
                mapper : (| parsed : NestedComplexInner | -> NestedComplexSpec {
                    let (flag ,
                    data) = parsed ;
                    NestedComplexSpec {
                        flag ,
    ...
    pub type GenericHeaderFmtSpec = Named < Mapped < Pair < U8 , Pair < U8 , Refined < U16Le , PredFnSpec < u16 >> > > , FnSpecMapper < GenericHeaderInner , GenericHeaderSpec >> > ;

    impl GenericHeaderFmt {
        # [doc = "specification constructor for `generic_header`."] pub open spec fn spec_inner () -> GenericHeaderFmtSpec {
            Named ("generic_header" ,
            Mapped {
                inner : Pair (U8 ,
                Pair (U8 ,
                Refined (U16Le ,
                | x : u16 | x >= 8 && x <= 65535))) ,
                mapper : (| parsed : GenericHeaderInner | -> GenericHeaderSpec {
                    let (next_type ,
                    (reserved ,
                    payload_length)) = parsed ;
                    GenericHeaderSpec {
                        next_type ,
                        reserved ,
                        payload_length
    ...
    pub type CombinedExampleFmtSpec = Named < Mapped < Bind < GenericHeaderFmt , spec_fn (GenericHeaderSpec) -> Varied < usize > > , FnSpecMapper < CombinedExampleInner , CombinedExampleSpec >> > ;

    impl CombinedExampleFmt {
        # [doc = "specification constructor for `combined_example`."] pub open spec fn spec_inner (total_len : u32) -> CombinedExampleFmtSpec {
            Named ("combined_example" ,
            Mapped {
                inner : Bind (GenericHeaderFmt ,
                | header : GenericHeaderSpec | Varied ((((total_len as usize) - (header . payload_length as usize)) as usize))) ,
                mapper : (| parsed : CombinedExampleInner | -> CombinedExampleSpec {
                    let (header ,
                    body) = parsed ;
                    CombinedExampleSpec {
                        header ,
    ...
    pub type PayloadWithHeaderFmtSpec = Named < Mapped < Bind < GenericHeaderFmt , spec_fn (GenericHeaderSpec) -> Varied < usize > > , FnSpecMapper < PayloadWithHeaderInner , PayloadWithHeaderSpec >> > ;

    impl PayloadWithHeaderFmt {
        # [doc = "specification constructor for `payload_with_header`."] pub open spec fn spec_inner () -> PayloadWithHeaderFmtSpec {
    ...
    pub type FinalMsgFmtSpec = Named < Mapped < Bind < Refined < U32Le , PredFnSpec < u32 >> , spec_fn (u32) -> Pair < CombinedExampleFmt , Bind < PayloadWithHeaderFmt , spec_fn (PayloadWithHeaderSpec) -> NestedComplexFmtSpec > > > , FnSpecMapper < FinalMsgInner , FinalMsgSpec >> > ;

    impl FinalMsgFmt {
        # [doc = "specification constructor for `final_msg`."] pub open spec fn spec_inner () -> FinalMsgFmtSpec {
            Named ("final_msg" ,
            Mapped {
                inner : Bind (Refined (U32Le ,
                | x : u32 | x >= 16777215 && x <= 4294967295) ,
                | total_len : u32 | Pair (CombinedExampleFmt :: spec (total_len) ,
                Bind (PayloadWithHeaderFmt ,
                | hdr_payload : PayloadWithHeaderSpec | NestedComplexFmt :: spec_inner (hdr_payload)))) ,
                mapper : (| parsed : FinalMsgInner | -> FinalMsgSpec {
                    let (total_len ,
                    (body ,
                    (hdr_payload ,
                    nested))) = parsed ;
    ...
    pub type OuterHeaderFmtSpec = Named < Mapped < Pair < U32Le , GenericHeaderFmt > , FnSpecMapper < OuterHeaderInner , OuterHeaderSpec >> > ;

    impl OuterHeaderFmt {
        # [doc = "specification constructor for `outer_header`."] pub open spec fn spec_inner () -> OuterHeaderFmtSpec {
            Named ("outer_header" ,
            Mapped {
                inner : Pair (U32Le ,
                GenericHeaderFmt) ,
                mapper : (| parsed : OuterHeaderInner | -> OuterHeaderSpec {
                    let (magic ,
                    inner) = parsed ;
                    OuterHeaderSpec {
                        magic ,
    ...
    pub type DeepNestedFmtSpec = Named < Mapped < Bind < OuterHeaderFmt , spec_fn (OuterHeaderSpec) -> Varied < usize > > , FnSpecMapper < DeepNestedInner , DeepNestedSpec >> > ;

    impl DeepNestedFmt {
        # [doc = "specification constructor for `deep_nested`."] pub open spec fn spec_inner () -> DeepNestedFmtSpec {
            Named ("deep_nested" ,
            Mapped {
                inner : Bind (OuterHeaderFmt ,
                | outer : OuterHeaderSpec | Varied ((((outer . inner . payload_length as usize) - 8) as usize))) ,
                mapper : (| parsed : DeepNestedInner | -> DeepNestedSpec {
                    let (outer ,
                    data) = parsed ;
                    DeepNestedSpec {
                        outer ,
    ...
    mod derived_specs {
        ...
        impl < 'i > SpecSerializerDps for NestedComplexFmt < 'i > {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                NestedComplexFmt :: spec_inner (self . hdr_payload_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for GenericHeaderFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                GenericHeaderFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for CombinedExampleFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                CombinedExampleFmt :: spec_inner (self . total_len_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for PayloadWithHeaderFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                PayloadWithHeaderFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for FinalMsgFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                FinalMsgFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for OuterHeaderFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                OuterHeaderFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for DeepNestedFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                DeepNestedFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
    ...
    mod exec_impls {
        ...
        impl < 'i > Parser < & 'i [u8] > for NestedComplexFmt < 'i > {
            ...
            fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
                ...
                let (n2 , data) = (Varied ((((self . hdr_payload . hdr . payload_length as usize) - 8 as usize) as usize))) . parse (& rest) ? ;
                let rest = rest.skip(n2);
                let total_n = n1 + n2;
                let final_v = NestedComplex {
            flag ,
            data
        }
        ;
        ...
        impl < 'i > Parser < & 'i [u8] > for CombinedExampleFmt {
            ...
            fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
                ...
                let rest = rest.skip(n1);
                let (n2 , body) = (Varied ((((self . total_len as usize) - (header . payload_length as usize)) as usize))) . parse (& rest) ? ;
                let rest = rest.skip(n2);
                let total_n = n1 + n2;
                let final_v = CombinedExample {
            header ,
            body
        }
        ;
        ...
        impl < 'i > Parser < & 'i [u8] > for PayloadWithHeaderFmt {
            ...
            fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
                ...
                let rest = rest.skip(n1);
                let (n2 , body) = (Varied ((((hdr . payload_length as usize) - 4 as usize) as usize))) . parse (& rest) ? ;
                let rest = rest.skip(n2);
                let total_n = n1 + n2;
        ...
        impl < 'i > Parser < & 'i [u8] > for FinalMsgFmt {
            ...
            return Err (ParseError :: predicate_failed ()) ;
                ...
                let rest = rest.skip(n1);
                let (n2 , body) = (CombinedExampleFmt {
            total_len : total_len
        }
        ) . parse (& rest) ? ;
                let rest = rest.skip(n2);
                ...
                let rest = rest.skip(n3);
                let (n4 , nested) = (NestedComplexFmt {
            hdr_payload : hdr_payload
        }
        ) . parse (& rest) ? ;
                let rest = rest.skip(n4);
                let total_n = n1 + n2 + n3 + n4;
                let final_v = FinalMsg {
            total_len ,
            body ,
            hdr_payload ,
            nested
        ...
        impl < 'i > Parser < & 'i [u8] > for DeepNestedFmt {
            ...
            fn parse(&self, ibuf: &&'i [u8]) -> PResult<Self::PT> {
                ...
                let rest = rest.skip(n1);
                let (n2 , data) = (Varied ((((outer . inner . payload_length as usize) - 8 as usize) as usize))) . parse (& rest) ? ;
                let rest = rest.skip(n2);
                let total_n = n1 + n2;
                let final_v = DeepNested {
            outer ,
            data
        }
        ;
