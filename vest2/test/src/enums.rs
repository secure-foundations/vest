...
verus! {
    ...
    impl < 'i > DeepView for ATypedChooseWithDefault < 'i > {
        ...
        open spec fn deep_view (& self) -> Self :: V {
            match self {
                ...
                ATypedChooseWithDefault :: Default (v) => ATypedChooseWithDefaultSpec :: Default (v . deep_view ()) ,
            }
        }
    }
    ...
    pub type ATypedChooseFmtSpec = Named < Mapped < Sum < U8 , Sum < U16Le , U32Le > > , FnSpecMapper < ATypedChooseInner , ATypedChooseSpec >> > ;

    impl ATypedChooseFmt {
        # [doc = "specification constructor for `a_typed_choose`."] pub open spec fn spec_inner (e : ATypedClosedEnumSpec) -> ATypedChooseFmtSpec {
            Named ("a_typed_choose" ,
            Mapped {
                inner : match e {
                ...
                ,
                mapper : (| parsed : ATypedChooseInner | -> ATypedChooseSpec {
                    match parsed {
    ...
    pub type ATypedOpenEnumFmtSpec = Named < Mapped < Choice < Refined < U32Le , PredFnSpec < u32 >> , Refined < U32Le , PredFnSpec < u32 >> > , FnSpecMapper < ATypedOpenEnumInner , ATypedOpenEnumSpec >> > ;

    impl ATypedOpenEnumFmt {
        # [doc = "specification constructor for `a_typed_open_enum`."] pub open spec fn spec_inner () -> ATypedOpenEnumFmtSpec {
            Named ("a_typed_open_enum" ,
            Mapped {
                inner : Choice (Refined (U32Le ,
                | x : u32 | x == 0 || x == 1 || x == 2) ,
                Refined (U32Le ,
                | x : u32 | x != 0 && x != 1 && x != 2)) ,
                mapper : (| parsed : ATypedOpenEnumInner | -> ATypedOpenEnumSpec {
                    match parsed {
                        L (x) => match x {
    ...
    pub type ANonDependentChooseFmtSpec = Named < Mapped < Choice < Refined < U8 , PredFnSpec < u8 >> , Choice < Refined < U8 , PredFnSpec < u8 >> , Refined < U8 , PredFnSpec < u8 >> > > , FnSpecMapper < ANonDependentChooseInner , ANonDependentChooseSpec >> > ;

    impl ANonDependentChooseFmt {
        # [doc = "specification constructor for `a_non_dependent_choose`."] pub open spec fn spec_inner () -> ANonDependentChooseFmtSpec {
            Named ("a_non_dependent_choose" ,
            Mapped {
                ...
                Refined (U8 ,
                | x : u8 | x >= 21))) ,
                mapper : (| parsed : ANonDependentChooseInner | -> ANonDependentChooseSpec {
                    match parsed {
    ...
    pub type ARegularChooseFmtSpec = Named < Mapped < Sum < U8 , Sum < U16Le , U32Le > > , FnSpecMapper < ARegularChooseInner , ARegularChooseSpec >> > ;

    impl ARegularChooseFmt {
        # [doc = "specification constructor for `a_regular_choose`."] pub open spec fn spec_inner (e : AClosedEnumSpec) -> ARegularChooseFmtSpec {
            Named ("a_regular_choose" ,
            Mapped {
                inner : match e {
                ...
                ,
                mapper : (| parsed : ARegularChooseInner | -> ARegularChooseSpec {
                    match parsed {
    ...
    pub type AMixedTypedEnumFmtSpec = Named < Mapped < Refined < U8 , PredFnSpec < u8 >> , FnSpecMapper < AMixedTypedEnumInner , AMixedTypedEnumSpec >> > ;

    impl AMixedTypedEnumFmt {
        # [doc = "specification constructor for `a_mixed_typed_enum`."] pub open spec fn spec_inner () -> AMixedTypedEnumFmtSpec {
            Named ("a_mixed_typed_enum" ,
            Mapped {
                inner : Refined (U8 ,
                | x : u8 | x == 0 || x == 1 || x == 2) ,
                mapper : (| parsed : AMixedTypedEnumInner | -> AMixedTypedEnumSpec {
                    match parsed {
    ...
    pub type AClosedEnumFmtSpec = Named < Mapped < Refined < U8 , PredFnSpec < u8 >> , FnSpecMapper < AClosedEnumInner , AClosedEnumSpec >> > ;

    impl AClosedEnumFmt {
        # [doc = "specification constructor for `a_closed_enum`."] pub open spec fn spec_inner () -> AClosedEnumFmtSpec {
            Named ("a_closed_enum" ,
            Mapped {
                inner : Refined (U8 ,
                | x : u8 | x == 0 || x == 1 || x == 2) ,
                mapper : (| parsed : AClosedEnumInner | -> AClosedEnumSpec {
                    match parsed {
                        0 => AClosedEnumSpec :: A ,
                        1 => AClosedEnumSpec :: B ,
                        2 => AClosedEnumSpec :: C ,
    ...
    pub type ATypedClosedEnumFmtSpec = Named < Mapped < Refined < U16Le , PredFnSpec < u16 >> , FnSpecMapper < ATypedClosedEnumInner , ATypedClosedEnumSpec >> > ;

    impl ATypedClosedEnumFmt {
        # [doc = "specification constructor for `a_typed_closed_enum`."] pub open spec fn spec_inner () -> ATypedClosedEnumFmtSpec {
            Named ("a_typed_closed_enum" ,
            Mapped {
                inner : Refined (U16Le ,
                | x : u16 | x == 0 || x == 1 || x == 2) ,
                mapper : (| parsed : ATypedClosedEnumInner | -> ATypedClosedEnumSpec {
                    match parsed {
    ...
    pub type AnOpenEnumFmtSpec = Named < Mapped < Choice < Refined < U8 , PredFnSpec < u8 >> , Refined < U8 , PredFnSpec < u8 >> > , FnSpecMapper < AnOpenEnumInner , AnOpenEnumSpec >> > ;

    impl AnOpenEnumFmt {
        # [doc = "specification constructor for `an_open_enum`."] pub open spec fn spec_inner () -> AnOpenEnumFmtSpec {
            Named ("an_open_enum" ,
            Mapped {
                inner : Choice (Refined (U8 ,
                | x : u8 | x == 0 || x == 1 || x == 2) ,
                Refined (U8 ,
                | x : u8 | x != 0 && x != 1 && x != 2)) ,
                mapper : (| parsed : AnOpenEnumInner | -> AnOpenEnumSpec {
                    match parsed {
                        L (x) => match x {
                            0 => AnOpenEnumSpec :: A ,
                            1 => AnOpenEnumSpec :: B ,
    ...
    impl ATypedChooseWithDefaultFmt {
        ...
        pub closed spec fn spec (e : ATypedOpenEnum) -> Self {
            ATypedChooseWithDefaultFmt {
                e
            }
        }
    }

    pub type ATypedChooseWithDefaultFmtSpec = Named < Mapped < Sum < U8 , Sum < U16Le , Sum < U32Le , Tail > > > , FnSpecMapper < ATypedChooseWithDefaultInner , ATypedChooseWithDefaultSpec >> > ;

    impl ATypedChooseWithDefaultFmt {
        # [doc = "specification constructor for `a_typed_choose_with_default`."] pub open spec fn spec_inner (e : ATypedOpenEnumSpec) -> ATypedChooseWithDefaultFmtSpec {
            Named ("a_typed_choose_with_default" ,
            Mapped {
                inner : match e {
                ...
                ,
                mapper : (| parsed : ATypedChooseWithDefaultInner | -> ATypedChooseWithDefaultSpec {
                    match parsed {
    ...
    impl AChooseWithDefaultFmt {
        ...
        pub closed spec fn spec (e : AnOpenEnum) -> Self {
            AChooseWithDefaultFmt {
                e
            }
        }
    }

    pub type AChooseWithDefaultFmtSpec = Named < Mapped < Sum < U8 , Sum < U16Le , Sum < U32Le , Tail > > > , FnSpecMapper < AChooseWithDefaultInner , AChooseWithDefaultSpec >> > ;

    impl AChooseWithDefaultFmt {
        # [doc = "specification constructor for `a_choose_with_default`."] pub open spec fn spec_inner (e : AnOpenEnumSpec) -> AChooseWithDefaultFmtSpec {
            Named ("a_choose_with_default" ,
            Mapped {
                inner : match e {
                ...
                ,
                mapper : (| parsed : AChooseWithDefaultInner | -> AChooseWithDefaultSpec {
                    match parsed {
    ...
    mod derived_specs {
        ...
        impl SpecSerializerDps for ATypedChooseFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ATypedChooseFmt :: spec_inner (self . e_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for ATypedOpenEnumFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ATypedOpenEnumFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for ANonDependentChooseFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ANonDependentChooseFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for ARegularChooseFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ARegularChooseFmt :: spec_inner (self . e_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for AMixedTypedEnumFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                AMixedTypedEnumFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for AClosedEnumFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                AClosedEnumFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for ATypedClosedEnumFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ATypedClosedEnumFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for AnOpenEnumFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                AnOpenEnumFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for ATypedChooseWithDefaultFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ATypedChooseWithDefaultFmt :: spec_inner (self . e_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for AChooseWithDefaultFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                AChooseWithDefaultFmt :: spec_inner (self . e_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
    ...
    mod exec_impls {
        ...
        impl < 'i > Parser < & 'i [u8] > for ANonDependentChooseFmt {
            ...
            va)) if va >= 0 && va <= 10 => {
                Ok ((n ,
                ANonDependentChoose :: Variant1 (va)))
            }
            ,
            _ => match (U8) . parse (& rest) {
                ...
                va)) if va >= 11 && va <= 20 => {
                    Ok ((n ,
                    ANonDependentChoose :: Variant2 (va)))
                }
                ,
                _ => match (U8) . parse (& rest) {
                    Ok ((n ,
                    va)) if va >= 21 => {
                        Ok ((n ,
                        ANonDependentChoose :: Variant3 (va)))
                    }
                    ,
