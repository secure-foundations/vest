...
verus! {
    ...
    impl < 'i > DeepView for Txout < 'i > {
        ...
        open spec fn deep_view (& self) -> Self :: V {
            TxoutSpec {
                ...
                script_pubkey : self . script_pubkey . deep_view () ,
            }
        }
    }
    ...
    impl < 'i > DeepView for Outpoint < 'i > {
        ...
        open spec fn deep_view (& self) -> Self :: V {
            OutpointSpec {
                hash : self . hash . deep_view () ,
                index : self . index . deep_view () ,
            }
        }
    }
    ...
    pub type TxSegwitInner = (u8 , (u64 , (Seq < TxinSpec > , (u64 , (Seq < TxoutSpec > , (Seq < WitnessSpec > , LockTimeSpec)))))) ;
    ...
    pub type ScriptFmtSpec = Named < Mapped < Bind < VarInt < true > , spec_fn (u64) -> Varied < usize > > , FnSpecMapper < ScriptInner , ScriptSpec >> > ;

    impl ScriptFmt {
        # [doc = "specification constructor for `script`."] pub open spec fn spec_inner () -> ScriptFmtSpec {
            Named ("script" ,
            Mapped {
                inner : Bind (VarInt :: < true > ,
                | l : u64 | Varied ((l as usize))) ,
                mapper : (| parsed : ScriptInner | -> ScriptSpec {
                    let (l ,
                    data) = parsed ;
                    ScriptSpec {
                        l ,
    ...
    impl TxoutFmt {
        # [doc = "specification constructor for `txout`."] pub open spec fn spec_inner () -> TxoutFmtSpec {
            Named ("txout" ,
            Mapped {
                inner : Pair (U64Le ,
                ScriptFmt) ,
                mapper : (| parsed : TxoutInner | -> TxoutSpec {
                    let (value ,
                    script_pubkey) = parsed ;
                    TxoutSpec {
                        value ,
                        script_pubkey
                    }
    ...
    pub type OutpointFmtSpec = Named < Mapped < Pair < Fixed < 32 > , U32Le > , FnSpecMapper < OutpointInner , OutpointSpec >> > ;

    impl OutpointFmt {
        # [doc = "specification constructor for `outpoint`."] pub open spec fn spec_inner () -> OutpointFmtSpec {
            Named ("outpoint" ,
            Mapped {
                inner : Pair (Fixed :: < 32 > ,
                U32Le) ,
                mapper : (| parsed : OutpointInner | -> OutpointSpec {
                    let (hash ,
                    index) = parsed ;
                    OutpointSpec {
                        hash ,
    ...
    pub type ScriptSigFmtSpec = Named < Mapped < Bind < VarInt < true > , spec_fn (u64) -> Varied < usize > > , FnSpecMapper < ScriptSigInner , ScriptSigSpec >> > ;

    impl ScriptSigFmt {
        # [doc = "specification constructor for `script_sig`."] pub open spec fn spec_inner () -> ScriptSigFmtSpec {
            Named ("script_sig" ,
            Mapped {
                inner : Bind (VarInt :: < true > ,
                | l : u64 | Varied ((l as usize))) ,
                mapper : (| parsed : ScriptSigInner | -> ScriptSigSpec {
                    let (l ,
                    data) = parsed ;
                    ScriptSigSpec {
                        l ,
    ...
    pub type WitnessComponentFmtSpec = Named < Mapped < Bind < VarInt < true > , spec_fn (u64) -> Varied < usize > > , FnSpecMapper < WitnessComponentInner , WitnessComponentSpec >> > ;

    impl WitnessComponentFmt {
        # [doc = "specification constructor for `witness_component`."] pub open spec fn spec_inner () -> WitnessComponentFmtSpec {
            Named ("witness_component" ,
            Mapped {
                inner : Bind (VarInt :: < true > ,
                | l : u64 | Varied ((l as usize))) ,
                mapper : (| parsed : WitnessComponentInner | -> WitnessComponentSpec {
                    let (l ,
                    data) = parsed ;
                    WitnessComponentSpec {
                        l ,
    ...
    pub type WitnessFmtSpec = Named < Mapped < Bind < VarInt < true > , spec_fn (u64) -> RepeatN < WitnessComponentFmt , usize > > , FnSpecMapper < WitnessInner , WitnessSpec >> > ;

    impl WitnessFmt {
        # [doc = "specification constructor for `witness`."] pub open spec fn spec_inner () -> WitnessFmtSpec {
            Named ("witness" ,
            Mapped {
                inner : Bind (VarInt :: < true > ,
                | count : u64 | RepeatN ((count as usize) ,
                WitnessComponentFmt)) ,
                mapper : (| parsed : WitnessInner | -> WitnessSpec {
                    let (count ,
                    data) = parsed ;
                    WitnessSpec {
                        count ,
    ...
    pub type TxinFmtSpec = Named < Mapped < Pair < OutpointFmt , Pair < ScriptSigFmt , U32Le > > , FnSpecMapper < TxinInner , TxinSpec >> > ;

    impl TxinFmt {
        # [doc = "specification constructor for `txin`."] pub open spec fn spec_inner () -> TxinFmtSpec {
            Named ("txin" ,
            Mapped {
                inner : Pair (OutpointFmt ,
                Pair (ScriptSigFmt ,
                U32Le)) ,
                mapper : (| parsed : TxinInner | -> TxinSpec {
                    let (previous_output ,
                    (script_sig ,
                    sequence)) = parsed ;
                    TxinSpec {
    ...
    pub type LockTimeFmtSpec = Named < Mapped < Choice < Refined < U32Le , PredFnSpec < u32 >> , Refined < U32Le , PredFnSpec < u32 >> > , FnSpecMapper < LockTimeInner , LockTimeSpec >> > ;

    impl LockTimeFmt {
        # [doc = "specification constructor for `lock_time`."] pub open spec fn spec_inner () -> LockTimeFmtSpec {
            Named ("lock_time" ,
            Mapped {
                inner : Choice (Refined (U32Le ,
                | x : u32 | x >= 0 && x <= 499999999) ,
                Refined (U32Le ,
                | x : u32 | x >= 500000000)) ,
                mapper : (| parsed : LockTimeInner | -> LockTimeSpec {
                    match parsed {
    ...
    pub type TxNonsegwitFmtSpec = Named < Mapped < Pair < RepeatN < TxinFmt , usize > , Bind < VarInt < true > , spec_fn (u64) -> Pair < RepeatN < TxoutFmt , usize > , LockTimeFmt > > > , FnSpecMapper < TxNonsegwitInner , TxNonsegwitSpec >> > ;

    impl TxNonsegwitFmt {
        # [doc = "specification constructor for `tx_nonsegwit`."] pub open spec fn spec_inner (txin_count : u64) -> TxNonsegwitFmtSpec {
            Named ("tx_nonsegwit" ,
            Mapped {
                inner : Pair (RepeatN ((txin_count as usize) ,
                TxinFmt) ,
                Bind (VarInt :: < true > ,
                | txout_count : u64 | Pair (RepeatN ((txout_count as usize) ,
                TxoutFmt) ,
                LockTimeFmt))) ,
                mapper : (| parsed : TxNonsegwitInner | -> TxNonsegwitSpec {
                    let (txins ,
                    (txout_count ,
                    (txouts ,
                    lock_time))) = parsed ;
    ...
    pub type TxSegwitFmtSpec = Named < Mapped < Pair < Const < U8 , u8 > , Bind < VarInt < true > , spec_fn (u64) -> Pair < RepeatN < TxinFmt , usize > , Bind < VarInt < true > , spec_fn (u64) -> Pair < RepeatN < TxoutFmt , usize > , Pair < RepeatN < WitnessFmt , usize > , LockTimeFmt > > > > > > , FnSpecMapper < TxSegwitInner , TxSegwitSpec >> > ;

    impl TxSegwitFmt {
        # [doc = "specification constructor for `tx_segwit`."] pub open spec fn spec_inner () -> TxSegwitFmtSpec {
            Named ("tx_segwit" ,
            Mapped {
                inner : Pair (Const (U8 ,
                1) ,
                Bind (VarInt :: < true > ,
                | txin_count : u64 | Pair (RepeatN ((txin_count as usize) ,
                TxinFmt) ,
                Bind (VarInt :: < true > ,
                | txout_count : u64 | Pair (RepeatN ((txout_count as usize) ,
                TxoutFmt) ,
                Pair (RepeatN ((txin_count as usize) ,
                WitnessFmt) ,
                LockTimeFmt)))))) ,
                mapper : (| parsed : TxSegwitInner | -> TxSegwitSpec {
                    let (flag ,
                    (txin_count ,
                    (txins ,
                    (txout_count ,
    ...
    pub type TxRemFmtSpec = Named < Mapped < Sum < TxSegwitFmt , TxNonsegwitFmt > , FnSpecMapper < TxRemInner , TxRemSpec >> > ;

    impl TxRemFmt {
        # [doc = "specification constructor for `tx_rem`."] pub open spec fn spec_inner (txin_count : u64) -> TxRemFmtSpec {
            Named ("tx_rem" ,
            Mapped {
                ...
                ,
                mapper : (| parsed : TxRemInner | -> TxRemSpec {
                    match parsed {
    ...
    pub type TxFmtSpec = Named < Mapped < Pair < U32Le , Bind < VarInt < true > , spec_fn (u64) -> TxRemFmt > > , FnSpecMapper < TxInner , TxSpec >> > ;

    impl TxFmt {
        # [doc = "specification constructor for `tx`."] pub open spec fn spec_inner () -> TxFmtSpec {
            Named ("tx" ,
            Mapped {
                inner : Pair (U32Le ,
                Bind (VarInt :: < true > ,
                | txin_count : u64 | TxRemFmt :: spec (txin_count))) ,
                mapper : (| parsed : TxInner | -> TxSpec {
                    let (version ,
                    (txin_count ,
                    rem)) = parsed ;
                    TxSpec {
    ...
    pub type BlockFmtSpec = Named < Mapped < Pair < U32Le , Pair < Fixed < 32 > , Pair < Fixed < 32 > , Pair < U32Le , Pair < U32Le , Pair < U32Le , Bind < VarInt < true > , spec_fn (u64) -> RepeatN < TxFmt , usize > > > > > > > > , FnSpecMapper < BlockInner , BlockSpec >> > ;

    impl BlockFmt {
        # [doc = "specification constructor for `block`."] pub open spec fn spec_inner () -> BlockFmtSpec {
            Named ("block" ,
            Mapped {
                inner : Pair (U32Le ,
                Pair (Fixed :: < 32 > ,
                ...
                Pair (U32Le ,
                Bind (VarInt :: < true > ,
                | tx_count : u64 | RepeatN ((tx_count as usize) ,
                TxFmt)))))))) ,
                mapper : (| parsed : BlockInner | -> BlockSpec {
                    let (version ,
                    (prev_block ,
                    (merkle_root ,
                    (timestamp ,
    ...
    mod derived_specs {
        ...
        impl Consistency for ScriptFmt {
            ...
            v : Self :: Val) -> bool {
                ScriptFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for ScriptFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ScriptFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for TxoutFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                TxoutFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for TxoutFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                TxoutFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for OutpointFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                OutpointFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for OutpointFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                OutpointFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for ScriptSigFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                ScriptSigFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for ScriptSigFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                ScriptSigFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for WitnessComponentFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                WitnessComponentFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for WitnessComponentFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                WitnessComponentFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for WitnessFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                WitnessFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for WitnessFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                WitnessFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for TxinFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                TxinFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for TxinFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                TxinFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for LockTimeFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                LockTimeFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for LockTimeFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                LockTimeFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for TxNonsegwitFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                TxNonsegwitFmt :: spec_inner (self . txin_count_spec ()) . consistent (v)
            }
        }
        impl SpecSerializerDps for TxNonsegwitFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                TxNonsegwitFmt :: spec_inner (self . txin_count_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for TxSegwitFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                TxSegwitFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for TxSegwitFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                TxSegwitFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for TxRemFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                TxRemFmt :: spec_inner (self . txin_count_spec ()) . consistent (v)
            }
        }
        impl SpecSerializerDps for TxRemFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                TxRemFmt :: spec_inner (self . txin_count_spec ()) . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl SpecSerializerDps for TxFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                TxFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
        ...
        impl Consistency for BlockFmt {
            ...
            open spec fn consistent (& self ,
            v : Self :: Val) -> bool {
                BlockFmt :: spec_inner () . consistent (v)
            }
        }
        impl SpecSerializerDps for BlockFmt {
            ...
            v : Self :: SValue ,
            obuf : Seq < u8 >) -> Seq < u8 > {
                BlockFmt :: spec_inner () . spec_serialize_dps (v ,
                obuf)
            }
        }
    ...
    mod derived_proofs {
        ...
        impl SafeParser for ScriptFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                ScriptFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for ScriptFmt {
            open spec fn productive_inv (& self) -> bool {
                ScriptFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< ScriptFmt as SpecParser > :: spec_parse) ;
                let fmt = ScriptFmt :: spec_inner () ;
        ...
        impl SafeParser for TxoutFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                TxoutFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for TxoutFmt {
            open spec fn productive_inv (& self) -> bool {
                TxoutFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< TxoutFmt as SpecParser > :: spec_parse) ;
                let fmt = TxoutFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for OutpointFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                OutpointFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for OutpointFmt {
            open spec fn productive_inv (& self) -> bool {
                OutpointFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< OutpointFmt as SpecParser > :: spec_parse) ;
                let fmt = OutpointFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for ScriptSigFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                ScriptSigFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for ScriptSigFmt {
            open spec fn productive_inv (& self) -> bool {
                ScriptSigFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< ScriptSigFmt as SpecParser > :: spec_parse) ;
                let fmt = ScriptSigFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for WitnessComponentFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                WitnessComponentFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for WitnessComponentFmt {
            open spec fn productive_inv (& self) -> bool {
                WitnessComponentFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< WitnessComponentFmt as SpecParser > :: spec_parse) ;
                let fmt = WitnessComponentFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for WitnessFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                WitnessFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for WitnessFmt {
            open spec fn productive_inv (& self) -> bool {
                WitnessFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< WitnessFmt as SpecParser > :: spec_parse) ;
                let fmt = WitnessFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for TxinFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                TxinFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for TxinFmt {
            open spec fn productive_inv (& self) -> bool {
                TxinFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< TxinFmt as SpecParser > :: spec_parse) ;
                let fmt = TxinFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for LockTimeFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                LockTimeFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for LockTimeFmt {
            open spec fn productive_inv (& self) -> bool {
                LockTimeFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< LockTimeFmt as SpecParser > :: spec_parse) ;
                let fmt = LockTimeFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for TxNonsegwitFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                TxNonsegwitFmt :: spec_inner (self . txin_count_spec ()) . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for TxNonsegwitFmt {
            open spec fn productive_inv (& self) -> bool {
                TxNonsegwitFmt :: spec_inner (self . txin_count_spec ()) . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< TxNonsegwitFmt as SpecParser > :: spec_parse) ;
                let fmt = TxNonsegwitFmt :: spec_inner (self . txin_count_spec ()) ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for TxSegwitFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                TxSegwitFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for TxSegwitFmt {
            open spec fn productive_inv (& self) -> bool {
                TxSegwitFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< TxSegwitFmt as SpecParser > :: spec_parse) ;
                let fmt = TxSegwitFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for TxRemFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                TxRemFmt :: spec_inner (self . txin_count_spec ()) . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for TxRemFmt {
            open spec fn productive_inv (& self) -> bool {
                TxRemFmt :: spec_inner (self . txin_count_spec ()) . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< TxRemFmt as SpecParser > :: spec_parse) ;
                let fmt = TxRemFmt :: spec_inner (self . txin_count_spec ()) ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for TxFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                TxFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for TxFmt {
            open spec fn productive_inv (& self) -> bool {
                TxFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< TxFmt as SpecParser > :: spec_parse) ;
                let fmt = TxFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
        ...
        impl SafeParser for BlockFmt {
            ...
            ibuf : Seq < u8 >) {
                ...
                BlockFmt :: spec_inner () . lemma_parse_safe (ibuf) ;
            }
        }
        impl Productive for BlockFmt {
            open spec fn productive_inv (& self) -> bool {
                BlockFmt :: spec_inner () . productive_inv ()
            }
            proof fn lemma_productive (& self ,
            s : Seq < u8 >) {
                reveal (< BlockFmt as SpecParser > :: spec_parse) ;
                let fmt = BlockFmt :: spec_inner () ;
                assert (fmt . productive_inv ()) ;
                fmt . lemma_productive (s) ;
            }
        }
