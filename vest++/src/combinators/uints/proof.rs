use super::spec::{u16be_combinator, u16le_combinator};
...
impl SPRoundTrip for super::U16Le {
    proof fn theorem_serialize_parse_roundtrip(&self, v: u16, obuf: Seq<u8>) {
        u16le_combinator().theorem_serialize_parse_roundtrip(v, obuf);
    }
}

impl PSRoundTrip for super::U16Le {
    proof fn theorem_parse_serialize_roundtrip(&self, ibuf: Seq<u8>, obuf: Seq<u8>) {
        u16le_combinator().theorem_parse_serialize_roundtrip(ibuf, obuf);
    }
}

impl NonMalleable for super::U16Le {
    proof fn lemma_parse_non_malleable(&self, buf1: Seq<u8>, buf2: Seq<u8>) {
        u16le_combinator().lemma_parse_non_malleable(buf1, buf2);
    }
}

impl Deterministic for super::U16Le {
    proof fn lemma_serialize_equiv(&self, v: u16, obuf: Seq<u8>) {
        u16le_combinator().lemma_serialize_equiv(v, obuf);
    }
}

impl SPRoundTrip for super::U16Be {
    proof fn theorem_serialize_parse_roundtrip(&self, v: u16, obuf: Seq<u8>) {
        u16be_combinator().theorem_serialize_parse_roundtrip(v, obuf);
    }
}

impl PSRoundTrip for super::U16Be {
    proof fn theorem_parse_serialize_roundtrip(&self, ibuf: Seq<u8>, obuf: Seq<u8>) {
        u16be_combinator().theorem_parse_serialize_roundtrip(ibuf, obuf);
    }
}

impl NonMalleable for super::U16Be {
    proof fn lemma_parse_non_malleable(&self, buf1: Seq<u8>, buf2: Seq<u8>) {
        u16be_combinator().lemma_parse_non_malleable(buf1, buf2);
    }
}

impl Deterministic for super::U16Be {
    proof fn lemma_serialize_equiv(&self, v: u16, obuf: Seq<u8>) {
        u16be_combinator().lemma_serialize_equiv(v, obuf);
    }
}
