...
proof fn test_star_3() {
    let c = (
        Star { inner: Refined { inner: Fixed::<1>, pred: |x: Seq<u8>| x[0] < 10u8 } },
        Fixed::<1>,
    );
    let obuf = Seq::empty();
...
proof fn test_star_5() {
    ...
    let nested_good = Star {
        inner: Refined { inner: Star { inner: Fixed::<1> }, pred: |x: Seq<Seq<u8>>| x.len() > 0 },
    };
...
proof fn test_star_varying_lengths() {
    ...
    let v = seq![(seq![1u8], seq![2u8, 3u8]), (seq![4u8], seq![5u8, 6u8])];
