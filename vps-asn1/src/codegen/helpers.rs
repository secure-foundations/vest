...
// Generated Rust and VPS syntax rendering.
...
pub(super) fn localize_rule_items(expression: &str, rule: EncodingRules) -> (String, Vec<String>) {
    let prefix = format!("vps_lib::asn1::{}::", rule.module());
...
pub(super) fn render_local_rule_import(rule: EncodingRules, items: &[String]) -> Option<String> {
    match items {
        [] => None,
        [item] => Some(format!("use vps_lib::asn1::{}::{item};", rule.module(),)),
        _ => Some(format!(
            "use vps_lib::asn1::{}::{{{}}};",
            rule.module(),
            items.join(", "),
...
pub(super) fn render_retag_helper(
    ...
    if let Some(rule) = qualified_explicit_rule {
        helper = format!("vps_lib::asn1::{}::{helper}", rule.module());
    }
...
mod tests {
    ...
    fn pretty_prints_sequence_and_balanced_choice_trees() {
        ...
        let (localized, items) = localize_rule_items(
            "vps_lib::asn1::ber::SEQUENCE(vps_lib::asn1::ber::BER_END)",
            EncodingRules::Ber,
        );
        ...
        assert_eq!(
            ...
            "use vps_lib::asn1::ber::{BER_END, SEQUENCE};"
        );
    }
}
