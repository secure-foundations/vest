...
`vps_asn1` parses ASN.1 modules with a vendored, locally patched
[`synta-codegen`](https://crates.io/crates/synta-codegen) frontend and emits BER
or DER codecs built directly from `vps_lib`'s verified ASN.1 primitives and
...
helper definitions. Verified bidirectional mappers hide VPS's nested tuple and
`Sum`s from users.

The nominal types delegate their specifications and executable operations to
the private combinator formats. `vps_lib::impl_der!` and
`vps_lib::impl_ber!` expose the proved parser, serializer, prepare, length,
