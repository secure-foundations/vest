...
fn assert_uses_local_start_certificates(generated: &str) {
    ...
    assert!(!generated.contains("broadcast use vps_lib::asn1::disjoint"));
    assert!(!generated
        .lines()
    ...
    assert!(generated.contains("vps_lib::impl_"));
...
"#,
    ...
    assert!(generated.contains("pub type Name = vps_lib::asn1::BmpString;"));
...
fn rejects_recursive_aliases_until_fixpoints_are_generated() {
    ...
    .unwrap_err();
    assert!(error.to_string().contains("VPS fixpoint combinator"));
}
...
fn emits_numeric_and_universal_strings_for_der_and_ber() {
    ...
    assert!(der.contains("pub type Digits<'a> = vps_lib::asn1::NumericString<'a>;"));
    assert!(der.contains("pub type Wide = vps_lib::asn1::UniversalString;"));

    let ber = compile_ber(schema).unwrap();
    assert!(ber.contains("pub type Digits = vps_lib::asn1::NumericStringOwned;"));
    assert!(ber.contains("pub type Wide = vps_lib::asn1::UniversalString;"));
...
fn mixed_rules_assign_one_transitive_rule_per_definition() {
    ...
    assert!(generated.contains("type CANONICAL__ = vps_lib::asn1::der::SetOfTlvFmt<SHARED>;"));
    assert!(generated.contains("use vps_lib::asn1::der::{DEFAULT, OCTET_STRING, SEQUENCE};"));
    assert!(generated.contains("use vps_lib::asn1::der::SET_OF;"));
    assert!(generated.contains("SET_OF(SHARED::Fmt)"));
    assert!(generated.contains("REQUIRED(Ref(SHARED::Fmt),"));
    assert!(!generated.contains("vps_lib::asn1::der::SET_OF("));
    assert!(generated.contains("type ORDERED__ = Mapped<vps_lib::asn1::der::SetFmt<"));
    assert!(generated.contains("pub type Version = i8;"));
...
fn generates_ber_formats_with_owned_flattened_values() {
    ...
    assert!(generated.contains("use vps_lib::asn1::ber::{"));
    assert!(generated.contains("pub type Payload = Vec<u8>;"));
    assert!(generated.contains("pub type Bits = vps_lib::asn1::BitStringOwned;"));
    assert!(generated.contains("pub type Label = String;"));
    assert!(generated.contains("pub type OpenValue = vps_lib::asn1::AnyOwned;"));
...
fn generates_ber_real_with_the_rule_specific_zero_copy_value() {
    ...
    assert!(generated.contains("pub type Measurement<'a> = vps_lib::asn1::Real<'a, BER>;"));
    assert!(generated.contains("type MEASUREMENT__ = RealTlvFmt;"));
...
fn generates_the_curated_cms_module_with_ber_and_canonical_der_substructures() {
    ...
    assert!(
        generated.contains("type SIGNED_ATTRIBUTES__ = Refined<vps_lib::asn1::der::SetOfTlvFmt<")
    );
    assert!(generated.contains("type CERTIFICATE__ = Mapped<vps_lib::asn1::der::"));
    assert!(generated.contains("type CONTENT_INFO__ = Mapped<vps_lib::asn1::ber::"));
...
fn parses_the_standalone_curated_cms_rfc5652_module() {
    use vps_asn1::ast::{TagClass, Tagging, TaggingMode};

    fn contains_only_lowered_wire_types(ty: &vps_asn1::Type) -> bool {
        use vps_asn1::Type;
    ...
    fn definition<'a>(module: &'a vps_asn1::SchemaModule, name: &str) -> &'a vps_asn1::Type {
        &module
            .definitions
            .iter()
    ...
    fn field<'a>(ty: &'a vps_asn1::Type, name: &str) -> &'a vps_asn1::Type {
        let vps_asn1::Type::Sequence(fields) = ty else {
    ...
    let module = vps_asn1::parse(include_str!("../rfcs/CMS-RFC5652-Curated.asn1")).unwrap();

    assert_eq!(module.name, "VpsCmsRfc5652Curated");
    ...
    let vps_asn1::Type::Tagged { tag, inner } =
    ...
    assert!(matches!(inner.as_ref(), vps_asn1::Type::Any));

    let vps_asn1::Type::Tagged { tag, .. } =
    ...
    let vps_asn1::Type::Choice(general_names) = definition(&module, "GeneralName") else {
    ...
    let directory_name = &general_names
        ...
        .ty;
    let vps_asn1::Type::Tagged { tag, .. } = directory_name else {
