...
impl UnsignedLEB128 {
    ...
    proof fn lemma_spec_parse_length(&self, s: Seq<u8>)
        ensures
            self.spec_parse(s) matches Ok((n, v)) ==> v <= 1 << (7 * n),
        decreases s.len(),
    {
        admit();
        // if s.len() != 0 {
    ...
    proof fn lemma_spec_parse_length_bound(&self, s: Seq<u8>)
        ensures
            self.spec_parse(s) matches Ok((n, v)) ==> n < 10,
        decreases s.len(),
    {
        admit();
    }
        ...
        if s.len() != 0 {
            ...
            if is_high_8_bit_set!(s[0]) {
                match self.exec_parse_rec_helper(s.subrange(1, s.len())) {
                    Ok((n, v2)) => {
                        // Check for overflow and canonicity (v2 should not be 0)
                        if n < usize::MAX && 0 < v2 && v2
                            <= n_bit_max_unsigned!(8 * uint_size!() - 7) {
                            Ok(((n + 1) as usize, v2 << 7 | v as UInt))
                        } else {
                            Err(
                                ParseError::Other(
                                    "Failed to parse LEB128: overflow or canonicity violation".to_string(),
                                ),
                            )
                        }
                    },
        ...
        if hi == 0 {
            ...
            proof {
                if let Ok(s_hi) = self.spec_serialize(hi@) {
                    assert(buf@ == seq_splice(
                        seq_splice(old(buf)@, (pos + 1) as usize, s_hi),
                        pos,
                        seq![set_high_8_bit!(lo)],
                    ));
                    assert_seqs_equal!(
    ...
    proof fn theorem_parse_serialize_roundtrip(&self, buf: Seq<u8>) {
        if let Ok((n, v)) = self.spec_parse(buf) {
            ...
            if is_high_8_bit_set!(buf.first()) {
                ...
                let first = buf.first();
                assert(first == take_low_7_bits!(first)) by (bit_vector)
                    requires
                        !is_high_8_bit_set!(first),
                ;
                assert(take_low_7_bits!(first) >> 7 == 0) by (bit_vector);
                assert(take_low_7_bits!(first) == take_low_7_bits!(take_low_7_bits!(first)))
                    by (bit_vector);
