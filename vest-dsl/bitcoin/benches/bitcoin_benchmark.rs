...
fn bench_parse_bitcoin_1k_blocks_bulk(c: &mut Criterion) {
    ...
    group.bench_function("rust_bitcoin", |b| b.iter(||
        for block_bytes in test_blocks.iter() {
...
fn bench_parse_bitcoin_1k_blocks(c: &mut Criterion) {
    ...
    for (i, test_block) in test_blocks.iter().enumerate() {
        group.bench_function(format!("vest_{}", i), |b| b.iter(||
            block().parse(black_box(test_block)).unwrap()
        ));
        group.bench_function(format!("rust_bitcoin_{}", i), |b| b.iter(||
...
fn bench_serialize_bitcoin_1k_blocks_bulk(c: &mut Criterion) {
    ...
    group.bench_function("vest", |b| b.iter_batched(
        || test_blocks.iter().map(|b| block().parse(b).unwrap().1).collect::<Vec<_>>(),
        |vest_parsed| {
    ...
    group.bench_function("rust_bitcoin", |b| b.iter_batched_ref(
        || test_blocks.iter().map(|block| bitcoin::Block::consensus_decode(&mut &block[..]).unwrap()).collect::<Vec<_>>(),
        |rust_btc_parsed| {
...
fn bench_serialize_bitcoin_1k_blocks(c: &mut Criterion) {
    ...
    for (i, test_block) in test_blocks.iter().enumerate() {
        group.bench_function(format!("vest_{}", i), |b| b.iter_batched(
        ...
        group.bench_function(format!("rust_bitcoin_{}", i), |b| b.iter_batched(
            || bitcoin::Block::consensus_decode(&mut &test_block[..]).unwrap(),
            |rust_block| {
                let mut buf = Vec::with_capacity(max_block_size);
