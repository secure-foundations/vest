...
impl SpecCombinator for SpecMsg4ContentCombinator {
    type SpecResult = SpecMsg4Content;
...
pub type SpecMsg4ContentCombinatorAlias = Mapped<
    OrdChoice<Cond<U16Le>, Cond<Tail>>,
    Msg4ContentMapper<'static>,
>;
...
impl<'a> View for Msg4ContentCombinator<'a> {
    ...
    closed spec fn view(&self) -> Self::V {
        SpecMsg4ContentCombinator(self.0@)
    }
}

impl<'a> Combinator<&'a [u8], Vec<u8>> for Msg4ContentCombinator<'a> {
    type Result = Msg4Content<'a>;
...
pub type Msg4ContentCombinatorAlias<'a> = Mapped<
    OrdChoice<Cond<U16Le>, Cond<Tail>>,
    Msg4ContentMapper<'a>,
>;

pub closed spec fn spec_msg4_content(i: u24) -> SpecMsg4ContentCombinator {
    SpecMsg4ContentCombinator(
        Mapped {
            inner: OrdChoice(
                Cond { cond: i.spec_as_u32() == 1, inner: U16Le },
                Cond { cond: !(i.spec_as_u32() == 1), inner: Tail },
            ),
            mapper: Msg4ContentMapper::spec_new(),
        },
    )
}
...
pub fn msg4_content<'a>(i: u24) -> (o: Msg4ContentCombinator<'a>)
    ensures
        o@ == spec_msg4_content(i@),
{
    Msg4ContentCombinator(
        Mapped {
            inner: OrdChoice::new(
                Cond { cond: i.as_u32() == 1, inner: U16Le },
                Cond { cond: !(i.as_u32() == 1), inner: Tail },
            ),
            mapper: Msg4ContentMapper::new(),
        },
    )
}
...
impl View for Msg4<'_> {
    ...
    open spec fn view(&self) -> Self::V {
        SpecMsg4 { i: self.i@, content: self.content@ }
    }
}
...
pub type SpecMsg4CombinatorAlias = Mapped<
    SpecDepend<U24Le, SpecMsg4ContentCombinator>,
    Msg4Mapper<'static>,
>;
...
pub type Msg4CombinatorAlias<'a> = Mapped<
    Depend<&'a [u8], Vec<u8>, U24Le, Msg4ContentCombinator<'a>, Msg4Cont<'a>>,
    Msg4Mapper<'a>,
>;
...
pub closed spec fn spec_msg4() -> SpecMsg4Combinator {
    SpecMsg4Combinator(
        Mapped {
            inner: SpecDepend { fst: U24Le, snd: |deps| spec_msg4_cont(deps) },
            mapper: Msg4Mapper::spec_new(),
        },
    )
}

pub open spec fn spec_msg4_cont(deps: u24) -> SpecMsg4ContentCombinator {
    let i = deps;
    spec_msg4_content(i)
}
...
impl<'a> Continuation<u24> for Msg4Cont<'a> {
...
impl SpecCombinator for SpecMsg5ContentCombinator {
    type SpecResult = SpecMsg5Content;
...
pub type SpecMsg5ContentCombinatorAlias = Mapped<
    OrdChoice<Cond<U16Le>, Cond<Tail>>,
    Msg5ContentMapper<'static>,
>;
...
impl<'a> View for Msg5ContentCombinator<'a> {
    ...
    closed spec fn view(&self) -> Self::V {
        SpecMsg5ContentCombinator(self.0@)
    }
}

impl<'a> Combinator<&'a [u8], Vec<u8>> for Msg5ContentCombinator<'a> {
    type Result = Msg5Content<'a>;
...
pub type Msg5ContentCombinatorAlias<'a> = Mapped<
    OrdChoice<Cond<U16Le>, Cond<Tail>>,
    Msg5ContentMapper<'a>,
>;

pub closed spec fn spec_msg5_content(i: VarInt) -> SpecMsg5ContentCombinator {
    SpecMsg5ContentCombinator(
        Mapped {
            inner: OrdChoice(
                Cond { cond: i.spec_as_usize() == 1, inner: U16Le },
                Cond { cond: !(i.spec_as_usize() == 1), inner: Tail },
            ),
            mapper: Msg5ContentMapper::spec_new(),
        },
    )
}
...
pub fn msg5_content<'a>(i: VarInt) -> (o: Msg5ContentCombinator<'a>)
    ensures
        o@ == spec_msg5_content(i@),
{
    Msg5ContentCombinator(
        Mapped {
            inner: OrdChoice::new(
                Cond { cond: i.as_usize() == 1, inner: U16Le },
                Cond { cond: !(i.as_usize() == 1), inner: Tail },
            ),
            mapper: Msg5ContentMapper::new(),
        },
    )
}
...
impl View for Msg5<'_> {
    ...
    open spec fn view(&self) -> Self::V {
        SpecMsg5 { i: self.i@, content: self.content@ }
    }
}

impl<'a> From<Msg5<'a>> for Msg5Inner<'a> {
    fn ex_from(m: Msg5<'a>) -> Msg5Inner<'a> {
        (m.i, m.content)
    }
}

impl<'a> From<Msg5Inner<'a>> for Msg5<'a> {
    fn ex_from(m: Msg5Inner<'a>) -> Msg5<'a> {
        let (i, content) = m;
        Msg5 { i, content }
    }
}
...
pub type SpecMsg5CombinatorAlias = Mapped<
    SpecDepend<BtcVarint, SpecMsg5ContentCombinator>,
    Msg5Mapper<'static>,
>;
...
pub type Msg5CombinatorAlias<'a> = Mapped<
    Depend<&'a [u8], Vec<u8>, BtcVarint, Msg5ContentCombinator<'a>, Msg5Cont<'a>>,
    Msg5Mapper<'a>,
>;
...
pub closed spec fn spec_msg5() -> SpecMsg5Combinator {
    SpecMsg5Combinator(
        Mapped {
            inner: SpecDepend { fst: BtcVarint, snd: |deps| spec_msg5_cont(deps) },
            mapper: Msg5Mapper::spec_new(),
        },
    )
}

pub open spec fn spec_msg5_cont(deps: VarInt) -> SpecMsg5ContentCombinator {
    let i = deps;
    spec_msg5_content(i)
}
    ...
    Msg5Combinator(
        Mapped {
            inner: Depend {
                fst: BtcVarint,
                snd: Msg5Cont::new(),
                spec_snd: Ghost(|deps| spec_msg5_cont(deps)),
            },
...
pub struct Msg5Cont<'a>(PhantomData<&'a ()>);

impl<'a> Msg5Cont<'a> {
    pub fn new() -> Self {
        Msg5Cont(PhantomData)
    }
}

impl<'a> Continuation<VarInt> for Msg5Cont<'a> {
