...
pub fn a_closed_enum_len<'a>(v: <AClosedEnumCombinator as Combinator<'a, &'a [u8], Vec<u8>>>::SType) -> (len: usize)
    requires
        spec_a_closed_enum().wf(v@),
        spec_a_closed_enum().spec_serialize(v@).len() <= usize::MAX,
    ensures
        len == spec_a_closed_enum().spec_serialize(v@).len(),
{
    let combinator = a_closed_enum();
...
pub closed spec fn spec_a_choose_with_default(e: u8) -> SpecAChooseWithDefaultCombinator {
...
pub fn a_choose_with_default_len<'a>(v: <AChooseWithDefaultCombinator as Combinator<'a, &'a [u8], Vec<u8>>>::SType, e: u8) -> (len: usize)
    requires
        spec_a_choose_with_default(e@).wf(v@),
        spec_a_choose_with_default(e@).spec_serialize(v@).len() <= usize::MAX,
    ensures
        len == spec_a_choose_with_default(e@).spec_serialize(v@).len(),
{
    let combinator = a_choose_with_default( e );
    <_ as Combinator<'a, &'a [u8], Vec<u8>>>::length(&combinator, v)
}
...
pub fn a_non_dependent_choose_len<'a>(v: <ANonDependentChooseCombinator as Combinator<'a, &'a [u8], Vec<u8>>>::SType) -> (len: usize)
    requires
        spec_a_non_dependent_choose().wf(v@),
        spec_a_non_dependent_choose().spec_serialize(v@).len() <= usize::MAX,
    ensures
        len == spec_a_non_dependent_choose().spec_serialize(v@).len(),
{
    let combinator = a_non_dependent_choose();
    <_ as Combinator<'a, &'a [u8], Vec<u8>>>::length(&combinator, v)
}
...
pub fn a_regular_choose_len<'a>(v: <ARegularChooseCombinator as Combinator<'a, &'a [u8], Vec<u8>>>::SType, e: AClosedEnum) -> (len: usize)
    requires
        spec_a_regular_choose(e@).wf(v@),
        spec_a_regular_choose(e@).spec_serialize(v@).len() <= usize::MAX,
    ensures
        len == spec_a_regular_choose(e@).spec_serialize(v@).len(),
{
    let combinator = a_regular_choose( e );
    <_ as Combinator<'a, &'a [u8], Vec<u8>>>::length(&combinator, v)
}
...
pub fn an_open_enum_len<'a>(v: <AnOpenEnumCombinator as Combinator<'a, &'a [u8], Vec<u8>>>::SType) -> (len: usize)
    requires
        spec_an_open_enum().wf(v@),
        spec_an_open_enum().spec_serialize(v@).len() <= usize::MAX,
    ensures
        len == spec_an_open_enum().spec_serialize(v@).len(),
{
    let combinator = an_open_enum();
    <_ as Combinator<'a, &'a [u8], Vec<u8>>>::length(&combinator, v)
}
