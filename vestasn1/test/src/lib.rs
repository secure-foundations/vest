...
mod tests {
    ...
    fn generated_der_formats_compose_all_ordering_wrappers() {
        assert_der_ord::<PayloadFmt, &'static [u8]>();
        assert_der_ord::<SelectionFmt, Selection<'static>>();
        assert_der_ord::<AutomationSequenceFmt, AutomationSequence<'static>>();
        assert_der_ord::<EnvelopesFmt, Vec<Envelope<'static>>>();
    }
    ...
    fn der_set_of_orders_complete_generated_tlvs_and_allows_duplicates() {
        ...
        let len = HEADERS_FMT().prepare(&values).unwrap();
        let mut encoded = vec![0; len];
        HEADERS_FMT().serialize(&values, encoded.as_mut_slice());
        ...
        assert!(HEADERS_FMT().prepare(&unordered).is_err());
        ...
        assert!(HEADERS_FMT().prepare(&duplicate).is_ok());
    }
    ...
    fn ber_generated_set_of_preserves_schema_order_without_der_sorting() {
        ...
        let mut output = vec![0; ber::FLAG_SET_FMT().prepare(&values).unwrap()];
        ber::FLAG_SET_FMT().serialize(&values, output.as_mut_slice());
        assert_eq!(output, [0x31, 0x06, 0x01, 0x01, 0xff, 0x01, 0x01, 0x00]);

        let (_, parsed) = ber::FLAG_SET_FMT().parse(&output.as_slice()).unwrap();
